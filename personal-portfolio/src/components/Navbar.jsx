...
export default function Navbar() {
  return (
    <nav className="fixed bottom-10 left-0 w-full flex justify-center items-center">
      <div className="backdrop:blur-16 border-4 border-neutral-600/70 backdrop-blur-md rounded-full px-17 py-3">
          ...
          {hrefs.map((item, index) => {
            return (
              <Link
                href={item.href}
                key={index}
                className="flex flex-col justify-center self-center items-center text-sm px-5 py-4 gap-0.5 hover:*: absolute w-0 h-0 border-[50%]"
              >
                {item.icon}
                <p>{item.name}</p>
              </Link>
